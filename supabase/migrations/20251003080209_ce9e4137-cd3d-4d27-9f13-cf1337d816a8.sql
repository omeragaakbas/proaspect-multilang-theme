...
-- 3. Fix profiles RLS - remove the duplicate SELECT policy that exposes all data
DROP POLICY IF EXISTS "Users can view limited public profiles" ON public.profiles;

-- Recreate a proper policy for viewing other users with restricted fields
-- Note: Application code must filter sensitive fields when displaying other users
